...
struct SphTriangle {
    ...
    Vector3 local_sample(curandState& rng) {
        Real r1 = curand_uniform(&rng);
        Real r2 = curand_uniform(&rng);
 ...
 * @return Vector3 
 */
Vector3 Triangle_sample(const Triangle* tri, curandState& rng, int which_part=-1);
 ...
 * @return Vector3 
 */
Vector3 Sphere_sample(const Sphere* sph, curandState& rng, int idx=0, int ct=1);
